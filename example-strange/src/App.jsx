...
const useStore = () => useVglobal();

const StorePumpkin = vGrip(({ pumpkin }) => {
...
const LocalPumpkin = vGrip(({ pumpkin, localActions }) => {
...
const PumpkinsNames = vGrip(({ names, changeFirstPumpkinColorInLocalToBlack }) => {
  ...
  return (
    <div style={{ border: '3px solid orange', padding: '10px', margin: '5px' }}>
      <div className="card">
        ...
        <button onClick={store.pumpkinStore.changeFirstPumpkinNameInStoreToOrange}>
          changeFirstPumpkinNameInStoreToOrange (SHOULD cause rerender - state.allPumpkinNames
          changed)
        </button>
        <p>Store pumpkins colors: {store.pumpkinStoreColors}</p>
        <button onClick={state.changeFirstPumpkinNameInLocalToYellow}>
          changeFirstPumpkinNameInLocalToYellow (SHOULD cause rerender - state.allPumpkinNames
          changed)
        </button>
